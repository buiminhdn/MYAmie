...
import { AccountStatus } from "@/models/app.interface";
import { useUpdateUserStatus } from "@/services/admin-user.service";
...
function AccountStatusChange({ id, status }: StatusChangeProps) {
  ...
  const dropdownOptions = useMemo(
    () => [
      { name: "Kích hoạt", id: AccountStatus.ACTIVATED },
      { name: "Khóa", id: AccountStatus.SUSPENDED },
    ],
    []
  );
  ...
  return (
    <>
      <button
        ...
        className="w-fit hover:underline hover:text-primary text-gray-500 font-medium hover:cursor-pointer"
      >
        Trạng thái
      </button>
      <ConfirmModal
